...
public class BillingCycleService : IBillingCycleService
{
    ...
    public async Task<ApiResponse<BillingCycleDto>> GetByIdAsync(int id)
    {
        var cycle = await _context
            .BillingCycles.Include(bc => bc.CreatedByUser)
    ...
    public async Task<ApiResponse<BillingCycleDto>> GetCurrentCycleAsync()
    {
        ...
        if (cycle == null)
        {
            return ApiResponse<BillingCycleDto>.ErrorResponse(
                "No billing cycle found for current period"
            );
        }
    ...
    {
        var query = _context.BillingCycles.Include(bc => bc.CreatedByUser).AsQueryable();
    ...
    public async Task<ApiResponse<BillingCycleDto>> CreateAsync(
        CreateBillingCycleDto dto,
        int userId
    )
    {
        // Check if cycle already exists
        if (
            await _context.BillingCycles.AnyAsync(bc =>
                bc.Month == dto.Month && bc.Year == dto.Year
            )
        )
        {
            return ApiResponse<BillingCycleDto>.ErrorResponse(
        ...
        cycle.CreatedByUser = await _userManager.FindByIdAsync(userId.ToString());

        return ApiResponse<BillingCycleDto>.SuccessResponse(
            await MapToDtoAsync(cycle),
    ...
    public async Task<ApiResponse<BillingCycleDto>> UpdateAsync(int id, UpdateBillingCycleDto dto)
    {
        ...
        if (
            ...
            && Enum.TryParse<BillingCycleStatus>(dto.Status, true, out var status)
        )
        {
            cycle.Status = status;
        }
    ...
    public async Task<ApiResponse<bool>> DeleteAsync(int id)
    {
        ...
        var hasData =
            await _context.MeterReadings.AnyAsync(m =>
                m.BillingMonth == cycle.Month && m.BillingYear == cycle.Year
            )
            || await _context.Bills.AnyAsync(b =>
                b.BillingMonth == cycle.Month && b.BillingYear == cycle.Year
            );

        if (hasData)
        {
            return ApiResponse<bool>.ErrorResponse(
                "Cannot delete billing cycle with existing readings or bills"
            );
        }
    ...
    {
        var totalReadings = await _context.MeterReadings.CountAsync(m =>
            m.BillingMonth == cycle.Month && m.BillingYear == cycle.Year
        );

        var totalBills = await _context.Bills.CountAsync(b =>
            b.BillingMonth == cycle.Month && b.BillingYear == cycle.Year
        );
        ...
        return new BillingCycleDto
        {
            ...
            CreatedByUserName =
                cycle.CreatedByUser != null
                    ? $"{cycle.CreatedByUser.FirstName} {cycle.CreatedByUser.LastName}"
                    : null,
